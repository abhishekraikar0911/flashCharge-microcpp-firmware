...
void OcppMeterService::resetScaling(unsigned long txStartTime) {
    ...
    // Set OCPP standard MeterValues to 60s (compliance interval)
    if (auto config = MicroOcpp::getConfigurationPublic("MeterValueSampleInterval")) {
        config->setInt(60);
    }
    Serial.printf("[METER_SVC] 📊 Initialized: Standard MV=60s, Compact Telemetry=%ds\n", _compactInterval);
}
...
void OcppMeterService::poll() {
    ...
    // ══════════════════════════════════════════════════════════════
    // STRATEGY 1: Keep OCPP MeterValues at 60s (GSM-safe, compliant)
    // ══════════════════════════════════════════════════════════════
    _meterStep = 2;
    _currentMvInterval = 60;
    static bool _forcedMvIntervalLogged = false;
    if (auto config = MicroOcpp::getConfigurationPublic("MeterValueSampleInterval")) {
        config->setInt(60);
        if (!_forcedMvIntervalLogged) {
            Serial.println("[METER_SVC] 📊 OCPP MeterValues locked at 60s (compliance)");
