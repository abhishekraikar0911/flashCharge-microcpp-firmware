...
// ═══════════════════════════════════════════════════════════════
// StopReason — centralized enumeration of all transaction stop causes.
// Use SystemState::setStopReason() from anywhere; logging is done
// in ONE place so all output is consistent and server-reportable.
// ═══════════════════════════════════════════════════════════════
enum class StopReason : uint8_t {
    NONE            = 0,  // No stop yet / session in progress
    REMOTE          = 1,  // CSMS-initiated RemoteStop
...
struct StateSnapshot {
    ...
    // ── Stop Reason ──  (last transaction stop cause)
...
public:
    ...
    double getEnergyWh()        { Lock l(_mutex); return _data.energyWh; }
    void  setEnergyWh(double v) { Lock l(_mutex); _data.energyWh = v; }
    void  addEnergyWh(double delta) { Lock l(_mutex); _data.energyWh += delta; }
    ...
    // --- Stop Reason ---
    // Called at every transaction termination to record + log the cause.
    // Prints a consistent [STOP] Reason: <X> line for all test monitoring.
    StopReason getStopReason() { Lock l(_mutex); return _data.lastStopReason; }
    void setStopReason(StopReason reason) {
        Lock l(_mutex);
        _data.lastStopReason = reason;
        Serial.printf("[STOP] Reason: %s\n", stopReasonStr(reason));
    }
    void clearStopReason() { Lock l(_mutex); _data.lastStopReason = StopReason::NONE; }
